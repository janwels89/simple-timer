...
class Display:
    ...
    def _draw_label_number(self, y, label, number, fill=0, base_number=None):
        label_font = self.font_label
        number_font = self.font_number
        small_font = self.font_status  # Use status font for base values
        ...
        # Position the main number
        if base_number is not None:
            # When showing base value, position main number a bit more to the left
            num_x = self.width - number_w - 35
        else:
        ...
        # Draw base number in smaller font if provided
        if base_number is not None and base_number != number:
            base_str = f"({base_number})"
            base_bbox = small_font.getbbox(base_str)
            base_w = base_bbox[2] - base_bbox[0]
            base_x = self.width - base_w - 5
            base_y = y + max(0, (label_h - (base_bbox[3] - base_bbox[1])) // 2) + 2
    ...
    def _render_status_and_numbers(self, open_num, close_num, status_a, status_b, status_c, open_base=None, close_base=None):
        ...
        self.draw = ImageDraw.Draw(self.image)
        self._draw_label_number(18, "OPEN", open_num, base_number=open_base)
        self._draw_label_number(42, "CLOSE", close_num, base_number=close_base)
    ...
    def update_values(self, timer, status_a=None, status_b=None, status_c=None):
        ...
        # Include base values if in random mode
        open_base = None
        close_base = None
        if hasattr(timer, "mode") and timer.mode == "random":
            open_base = getattr(timer, "open_time_base", None)
            close_base = getattr(timer, "close_time_base", None)
        ...
        if current_state != self._last_state:
            logger.debug(
                ...
                curr_a, curr_b, curr_c, open_base, close_base
            )
            self._last_state = current_state.copy()
